...
world_bank_pop_total  <- world_bank_pop %>% 
  filter( country %in% c("TUN", "MAR", "DZA", "ESP") & indicator == "SP.POP.TOTL") %>%
  select(- indicator) 
...
world_bank_pop_total_longer <- world_bank_pop_total  %>% 
  pivot_longer(cols = `2000`:`2017`,names_to = "Year", values_to = "population", names_transform = list(Year = as.integer)) 
