...
```
Nous remarquons que la transparence des points est cumulative : les zones avec un degre eleve de chevauchement sont plus sombres.
...
```{r}
ggplot(data = alaska_flights, mapping = aes(x = dep_delay, y = arr_delay)) +  
  geom_jitter(width = 30, height = 40)
...
```{r}
ggplot(data = january_weather, mapping = aes(x = time_hour, y = temp)) +
  geom_line()
```
...
```{r}
ggplot(data = weather, mapping = aes(x = temp)) +
  geom_histogram(binwidth = 10 , color = "white" ,fill = "royalblue")
```
...
Pour examiner la distribution de la variable `temp`, produiser boxplot via le  mapping de la variable `temp`  au paramètre y au niveau des aesthetic et l'utilsaition du geom adequat.
...
```{r}
ggplot(data = weather) +
  geom_boxplot(aes(x = factor(month), y = temp), fill = "wheat", color = "brown")
```
...
```{r}
ggplot(data = flights, mapping = aes(x = carrier)) +
  geom_bar(fill = "skyblue", color = "royalblue") + 
  facet_grid(origin~.)
